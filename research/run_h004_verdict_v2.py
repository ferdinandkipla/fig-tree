...
def load_pooled_train_trades(symbol: str, timeframe: str) -> pd.DataFrame:
    ...
    for seed in range(N_NULL_SEEDS):
        path = Path(f"research/null_runs_h004_v2/{symbol}_{timeframe}_seed{seed}_trades.csv")
        if not path.exists():
            continue
...
if __name__ == "__main__":
    ...
    combined.to_csv("research/H-004-verdict-v2.csv", index=False)
    ...
    print(f"\nSaved -> research/H-004-verdict-v2.csv")

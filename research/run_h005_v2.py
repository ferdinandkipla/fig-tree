...
# (research/null_runs_h004_v2/), which already captures atr_entry.
#
# Usage:
...
NULL_RUNS_DIR = Path("research/null_runs_h004_v2")
...
def freeze_bins():
    ...
    try:
        entry = record(
            ...
            output_paths={s: f"research/null_runs_h004_v2/{s}_H1_seed0_trades.csv" for s in SYMBOLS},
...
def compute_verdict():
    ...
    verdict_df.to_csv("research/H-005-verdict-v2.csv", index=False)
    survives = consistent_best and n_sig_and_real >= 4
    print(f"\nPRIMARY CLAIM {'SURVIVES this check' if survives else 'KILLED'}")
    print(f"Saved -> research/H-005-verdict-v2.csv")
    return verdict_df

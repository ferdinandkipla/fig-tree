...
def _load_train_trades(symbol: str) -> pd.DataFrame:
    df = pd.read_csv(f"research/patchonly_v2/trades_{symbol}.csv", parse_dates=["entry_dt"])
...
def compute_verdict():
    ...
    for symbol in SYMBOLS:
        ...
        for seed in range(N_NULL_SEEDS):
            path = Path(f"research/null_runs_v2/{symbol}_seed{seed}_trades.csv")
            if not path.exists():
                continue
    ...
    verdict_df.to_csv("research/H-001-verdict-v2.csv", index=False)
    print("\n" + "=" * 90)
    print("H-001 VERDICT TABLE (TRAIN 2019-2022, bins frozen, read-only in this phase)")
    print("=" * 90)
    print(verdict_df.to_string(index=False))
    print(f"\nSaved -> research/H-001-verdict-v2.csv")
    return verdict_df

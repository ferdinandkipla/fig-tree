...
| `spread_cost`/`slip_cost` formula in `execution/costs.py` — unit/double-conversion bug | **FIXED (this commit).** Confirmed via `tests/test_cost_model_spec.py` (written before the fix, failed against the buggy code as expected, all 6 pass after) — `pip_size` was applied twice, making spread/slip ~10,000x too small for EURUSD. Fixed by removing the extraneous `pip_size` multiplication (`pip_value` is already dollars-per-pip-per-lot, confirmed against its use in `execution/simulator.py`/`risk/sizing.py`). Full record: `docs/COST_MODEL_V3_SPREAD_FIX.md`. **Consequence: every cost +50% stress test H-001 through H-009 ran under a near-zero spread/slip term.** Verdict-flip risk assessed as low (kill-asymmetry argument: a larger correct cost can only strengthen a kill) but NOT yet re-verified — see the re-verification staleness flag above, now three cost-model changes (swap refresh, commission, this fix) removed from what `a42a7aa` actually tested. |
